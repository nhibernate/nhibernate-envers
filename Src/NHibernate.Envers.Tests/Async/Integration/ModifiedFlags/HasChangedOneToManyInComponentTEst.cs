...
namespace NHibernate.Envers.Tests.Integration.ModifiedFlags
{
	...
	public partial class HasChangedOneToManyInComponentTest : AbstractModifiedFlagsEntityTest
	{
		...
		{
			(await (QueryForPropertyHasChangedAsync(typeof (OneToManyComponentTestEntity), id, "Comp1")).ConfigureAwait(false))
				.ExtractRevisionNumbersFromRevision()
				.Should().Have.SameSequenceAs(2, 3);

			(await (QueryForPropertyHasNotChangedAsync(typeof (OneToManyComponentTestEntity), id, "Comp1")).ConfigureAwait(false))
				.ExtractRevisionNumbersFromRevision()
				.Should().Be.Empty();
		}
	}
