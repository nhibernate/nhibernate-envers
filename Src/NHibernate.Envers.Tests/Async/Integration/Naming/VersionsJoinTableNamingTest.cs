...
namespace NHibernate.Envers.Tests.Integration.Naming
{
	...
	public partial class VersionsJoinTableNamingTest : TestBase
	{
		...
		{
			CollectionAssert.AreEquivalent(new[] { 1, 2 }, await (AuditReader().GetRevisionsAsync(typeof(VersionsJoinTableTestEntity), uni1_id)).ConfigureAwait(false));
		...
		public async Task VerifyHistoryOfUniId1Async()
		{
			...
			var rev1 = await (AuditReader().FindAsync<VersionsJoinTableTestEntity>(uni1_id, 1)).ConfigureAwait(false);
			var rev2 = await (AuditReader().FindAsync<VersionsJoinTableTestEntity>(uni1_id, 2)).ConfigureAwait(false);
