...
namespace NHibernate.Envers.Tests.Integration.OneToMany.Detached
{
	...
	public partial class BasicDetachedSetTest : TestBase
	{
		...
		{
			CollectionAssert.AreEquivalent(new[] { 1, 2, 3, 4 }, await (AuditReader().GetRevisionsAsync(typeof(SetRefCollEntity), coll1_id)).ConfigureAwait(false));
		...
		public async Task VerifyHistoryOfColl1Async()
		{
			...
			var rev3 = await (AuditReader().FindAsync<SetRefCollEntity>(coll1_id, 3)).ConfigureAwait(false);
			var rev4 = await (AuditReader().FindAsync<SetRefCollEntity>(coll1_id, 4)).ConfigureAwait(false);
