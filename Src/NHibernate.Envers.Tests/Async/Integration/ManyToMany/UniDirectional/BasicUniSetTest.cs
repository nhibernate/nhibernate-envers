...
namespace NHibernate.Envers.Tests.Integration.ManyToMany.UniDirectional
{
	...
	public partial class BasicUniSetTest : TestBase
	{
		...
		public async Task VerifyRevisionCountAsync()
		{
			...
			CollectionAssert.AreEquivalent(new[] { 1, 2, 3, 4, 5 }, await (AuditReader().GetRevisionsAsync(typeof(SetUniEntity), ing1_id)).ConfigureAwait(false));
			CollectionAssert.AreEquivalent(new[] { 1, 2 }, await (AuditReader().GetRevisionsAsync(typeof(SetUniEntity), ing2_id)).ConfigureAwait(false));
		}
		...
		public async Task VerifyHistoryIng1Async()
		{
			...
			var rev1 = await (AuditReader().FindAsync<SetUniEntity>(ing1_id, 1)).ConfigureAwait(false);
			var rev2 = await (AuditReader().FindAsync<SetUniEntity>(ing1_id, 2)).ConfigureAwait(false);
			var rev3 = await (AuditReader().FindAsync<SetUniEntity>(ing1_id, 3)).ConfigureAwait(false);
			var rev4 = await (AuditReader().FindAsync<SetUniEntity>(ing1_id, 4)).ConfigureAwait(false);
			var rev5 = await (AuditReader().FindAsync<SetUniEntity>(ing1_id, 5)).ConfigureAwait(false);
		...
		public async Task VerifyHistoryIng2Async()
		{
			...
			var rev1 = await (AuditReader().FindAsync<SetUniEntity>(ing2_id, 1)).ConfigureAwait(false);
			var rev2 = await (AuditReader().FindAsync<SetUniEntity>(ing2_id, 2)).ConfigureAwait(false);
			var rev3 = await (AuditReader().FindAsync<SetUniEntity>(ing2_id, 3)).ConfigureAwait(false);
			var rev4 = await (AuditReader().FindAsync<SetUniEntity>(ing2_id, 4)).ConfigureAwait(false);
			var rev5 = await (AuditReader().FindAsync<SetUniEntity>(ing2_id, 5)).ConfigureAwait(false);
