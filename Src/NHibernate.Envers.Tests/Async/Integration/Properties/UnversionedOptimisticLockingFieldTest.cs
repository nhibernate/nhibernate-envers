...
namespace NHibernate.Envers.Tests.Integration.Properties
{
	...
	public partial class UnversionedOptimisticLockingFieldTest : TestBase
	{
		...
		{
			(await (AuditReader().GetRevisionsAsync(typeof (UnversionedOptimisticLockingFieldEntity), id)).ConfigureAwait(false))
		...
		public async Task VerifyHistoryAsync()
		{
			...
			(await (AuditReader().FindAsync<UnversionedOptimisticLockingFieldEntity>(id, 1)).ConfigureAwait(false))
				.Should().Be.EqualTo(ver1);
			(await (AuditReader().FindAsync<UnversionedOptimisticLockingFieldEntity>(id, 2)).ConfigureAwait(false))
