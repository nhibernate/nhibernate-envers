...
namespace NHibernate.Envers.Tests.NetSpecific.Integration.DynamicComponent.JoinWithDynamicComponent
{
	...
	public partial class BasicJoinWithDynamicComponentTest : TestBase
	{
		...
		private async Task loadDataOnSessionAndAuditReaderAsync(CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			currentPerson1 = (Person)await (Session.GetAsync("Person", id_pers1, cancellationToken)).ConfigureAwait(false);
		...
		private async Task checkEntitiesAsync(CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			var person2 = (Person)await (Session.GetAsync("Person", id_pers2, cancellationToken)).ConfigureAwait(false);
