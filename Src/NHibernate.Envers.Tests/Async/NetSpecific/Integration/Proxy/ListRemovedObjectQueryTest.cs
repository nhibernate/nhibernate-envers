...
namespace NHibernate.Envers.Tests.NetSpecific.Integration.Proxy
{
	...
	public partial class ListRemovedObjectQueryTest : TestBase
	{
		...
		{
			(await (AuditReader().FindAsync<ListParent>(parentId, 1)).ConfigureAwait(false))
						 .Children.Single().Name.Should().Be.EqualTo("child1");
			(await (AuditReader().FindAsync<ListParent>(parentId, 2)).ConfigureAwait(false))
						 .Children.Single().Name.Should().Be.EqualTo("child12");
			var rev3 = await (AuditReader().CreateQuery().ForRevisionsOfEntity(typeof(ListParent), true, true)
			             .Add(AuditEntity.Id().Eq(parentId))
			             .Add(AuditEntity.RevisionNumber().Eq(3))
			             .GetSingleResultAsync<ListParent>()).ConfigureAwait(false);
