...
{
	[TestFixture]
	public class DoubleDetachedSetTest : TestBase
	{
		private int str1_id;
		private int str2_id;
		...
		protected override void Initialize()
		{
			...
			using (var tx = Session.BeginTransaction())
			{
				...
				coll1.Collection = new HashedSet<StrTestEntity> {str1};
				Session.Save(coll1);
				coll1.Collection2 = new HashedSet<StrTestEntity> {str2};
				Session.Save(coll1);
				tx.Commit();
			}
			using (var tx = Session.BeginTransaction())
		...
		{
			CollectionAssert.AreEquivalent(new[] { 1, 2, 3 }, AuditReader().GetRevisions<DoubleSetRefCollEntity>( coll1_id));
