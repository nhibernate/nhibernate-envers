...
namespace NHibernate.Envers.Tests.Integration.OneToMany.Detached
{
	...
	public class BasicDetachedListTest : TestBase
	{
		...
		protected override void Initialize()
		{
			...
			var coll1 = new ListRefCollEntity {Id = coll1_id, Data = "coll1"};

			using (var tx = Session.BeginTransaction())
			{
				str1_id = (int) Session.Save(str1);
				str2_id = (int) Session.Save(str2);
				coll1.Collection = new List<StrTestEntity> {str1};
				Session.Save(coll1);
				tx.Commit();
			}
			using (var tx = Session.BeginTransaction())
		...
		{
			CollectionAssert.AreEquivalent(new[] { 1, 2, 3, 4 }, AuditReader().GetRevisions<ListRefCollEntity>( coll1_id));
		...
		public void VerifyHistoryOfColl1()
		{
			...
			var rev1 = AuditReader().Find<ListRefCollEntity>(coll1_id, 1);
			var rev2 = AuditReader().Find<ListRefCollEntity>(coll1_id, 2);
			var rev3 = AuditReader().Find<ListRefCollEntity>(coll1_id, 3);
			var rev4 = AuditReader().Find<ListRefCollEntity>(coll1_id, 4);
