...
namespace NHibernate.Envers.Tests.Integration.Collection
{
	public class StringMapTest : TestBase
	{
		...
		{
			CollectionAssert.AreEquivalent(new[] { 1, 2, 3 }, AuditReader().GetRevisions<StringMapEntity>( sme1_id));
			CollectionAssert.AreEquivalent(new[] { 1, 3 }, AuditReader().GetRevisions<StringMapEntity>( sme2_id));
		}
