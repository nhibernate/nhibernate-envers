...
namespace NHibernate.Envers.Tests.Integration.Join
{
	...
	public class BasicJoinTest : TestBase
	{
		...
		{
			CollectionAssert.AreEquivalent(new[]{1, 2}, AuditReader().GetRevisions<JoinTestEntity>( id));
		}
		...
		{
			const string auditName = TestAssembly + ".Integration.Join.JoinTestEntity_AUD";
