...
namespace NHibernate.Envers.Tests.NetSpecific.Integration.OneToOne
{
	...
	public class OneToOnePrimaryKeyTest : TestBase
	{
		...
		{
			CollectionAssert.AreEquivalent(new[] { 1, 2, 3, 4 }, AuditReader().GetRevisions<OneToOneOwningEntity>( id));
			CollectionAssert.AreEquivalent(new[] { 2, 3, 4}, AuditReader().GetRevisions<OneToOneOwnedEntity>(id));
		}
